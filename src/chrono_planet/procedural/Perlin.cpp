...
float noise(float x, float y) {
    ...
    x -= std::floor(x);
    y -= std::floor(y);
    const float fadeX = qtplanet::smootherstep01(x), fadeY = qtplanet::smootherstep01(y);
...
float onSphere(const qtplanet::Vec3& unitDirection, float coordinateScale) {
...
void writeNoiseRow(const float* x, const float* y, int sampleCount, float* noiseValues) {
    ...
    while (i < sampleCount) {
        ...
        for (int k = i; k < end; ++k) {
            ...
            const float fadeX = qtplanet::smootherstep01(fractionX), fadeY = qtplanet::smootherstep01(fractionY);
