...
double valueNoise3(Vec3 p, int seed) {
    const int ix = qtfield::fastFloor(p.x), iy = qtfield::fastFloor(p.y), iz = qtfield::fastFloor(p.z);
    const double fx = p.x - ix, fy = p.y - iy, fz = p.z - iz;
    const double u = qtplanet::smoothstep01(fx), v = qtplanet::smoothstep01(fy),
                 w = qtplanet::smoothstep01(fz);
    double acc = 0.0;
...
std::vector<SlicePlane> slicePlanes(int seed) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        const double phi = qtplanet::kTwoPi * hash01(seed, 20 + i, 7020);
...
std::vector<Vec3> displacedPositions(int seed, const std::vector<Vec3>& directions) {
    ...
    // Keep RockInstance::radiusM equal to the maximum distance from the origin.
...
RockMesh::Lod appendLod(const std::vector<Vec3>& positions, const IcosphereLevel& level, RockMesh& mesh) {
    ...
    const double creaseCosine = std::cos(qtplanet::deg2rad(kCreaseDeg));
